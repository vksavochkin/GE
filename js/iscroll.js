...
/* REPLACE END: _move */
	...
	_end: function (e) {
		...
		if ( !this.moved ) {
			...
			if ( this.options.click && !utils.isBadAndroid ) {
				utils.click(e);
			}
...
function makeScroll(id){//https://github.com/cubiq/iscroll/pull/548
	return true;
	try{
		if(!Check.isEmpty(scrollers[id] )){
			//console.log(scrollers[id]);
			scrollers[id].destroy();
			...
			//scrollers[id].refreshOnContentResize();
		}
...
function toScroll(id,y,x){
return true;
	if(!Check.isEmpty(scrollers[id] )){
		scrollers[id].scrollTo(y, x, 200);
	}	
}

function toScrollElement(id,el,s){
return true;
	if(!Check.isEmpty(scrollers[id] )){
		scrollers[id].scrollToElement(el, s);
	}	
}
